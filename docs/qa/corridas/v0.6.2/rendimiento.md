# Corrida de rendimiento — `v0.6.2` (E33-H04 / E33-H07)
...
Esta corrida reutiliza el informe oficial H04 del 2026-08-22 porque es trazable al commit
`0c9d0ba7980a0d1059d918844ab3864f361b661b`, descendiente del commit de release
`73251af05fa2c4589447294719854801a23ae4aa` de `v0.6.2`, y fue
ejecutada con `build_profile=release`. El JSON completo es un artefacto comprimido externo
inventariado en [`manifest.json`](manifest.json); sus raíces temporales se representan como
`<ephemeral-root>` para no versionar rutas privadas.
...
## Gate E33-H05 ejecutado para el release

Comando ejecutado con el binario release en la máquina de baseline:

```text
target/release/lodestar-bench --gate --report /ruta/temporal/e33-h07-v0.6.2-rendimiento.json \
...
**PASS — modo absoluto**, perfil `realista`, variante `disk-reparseo`, escala `10000`.
Los siete p95 y `cold-open` quedan bajo los techos ratificados; el gate terminó con exit `0`.
