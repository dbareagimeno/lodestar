...
2. **Lanza el juez** (tipo `juez-historia`, agente NUEVO — nunca un fork, nunca `SendMessage` a un
   ...
   **PROHIBIDO** incluir en el prompt: por qué se implementó así, resúmenes de la conversación,
   ...
   ser ciego.
3. **`--panel`**: lanza 3 jueces `juez-historia` frescos **en paralelo**, mismo expediente, cada uno
   con una lente declarada al inicio de su prompt:
...
4. **Sintetiza** (solo con `--panel`): el veredicto agregado es el **peor** de los tres (un
   RECHAZADA de cualquier lente rechaza el conjunto). Deduplica hallazgos y presenta la tabla
